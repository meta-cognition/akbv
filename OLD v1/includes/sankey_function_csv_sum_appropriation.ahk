csv_find_and_sum_appropriation(string_find, search_column)
{
	...
	{	
		...
		{
			if (line_type = "AP" || line_type = "AL" )
			{
				sum += cell_value   
				;FileAppend, % string_find ": numbers sum: " sum ", cell: " cell_value "`r`n", dump_sankey.txt
			}
		}
		current_csv_row++
	}
	;FileAppend, % "approp sum: " sum ", readable: " format_to_readable(sum) "`r`n", dump_sankey.txt
