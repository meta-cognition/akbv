		...
		loop, % sankey_object.nodes.columns[target_column].rows.length()
		{
			...
			loop, % sankey_object.nodes.columns[grouping_column].rows.length()
			{
				...
				if (this_link_value > 0)
				{
					...
					{
						format_mode := "dollars"
						this_link_object.link_label := format_to_readable(this_link_value) . " to " .  sankey_object.nodes.columns[target_column].rows[current_target_node_row].node_name . "<br />Fund Source: " . sankey_object.nodes.columns[grouping_column].rows[current_grouping_row].node_name . " " . fund_type_short(sankey_object.nodes.columns[grouping_column].rows[current_grouping_row].node_name)
